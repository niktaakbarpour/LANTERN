...
def build_target_db(base_dir, it):
    ...
    def yaml_like_to_json(text):
        """Convert '- Key: Value' lines into a JSON dict string."""
        data = {}
        ...
        if filename.endswith('.json'):
            ...
            if code_id not in target_db:
                ...
                try:
                    ...
                    print(f"[WARNING] Non-JSON content found for {code_id}. Attempting conversion.")
                    code_json = yaml_like_to_json(code)
                    if not code_json:
                        print(f"[ERROR] Could not parse response for {code_id}: {code}")
                        continue

                # Get target language
                target_lang = code_json.get("Target Language")
                if not target_lang:
                    print(f"[ERROR] 'Target Language' missing for {code_id}")
                    continue
...
def retrieve_base(base_dir, it, bug_code_uid, top_k, initial_unfixed_ids, bug_properties, cos, print_mode=False):
    ...
    if len(similarities_tight) < top_k:
        ...
        print(f"Nikta similarities 1: {similarities}")
    else:
        similarities = similarities_tight
        print(f"Nikta similarities 2: {similarities}")
...
def process_df(query_df, similar_bugs_df, similar_bugs_t_df, print_mode=False, nohist=False):
    ...
    print(f"repair_hist: {repair_hist} ({type(repair_hist)})")
    print(f"trans_repair_hist: {trans_repair_hist} ({type(trans_repair_hist)})")
